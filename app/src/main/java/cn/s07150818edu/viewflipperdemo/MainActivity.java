package cn.s07150818edu.viewflipperdemo;
...
public class MainActivity extends AppCompatActivity implements GestureDetector.OnGestureListener{
    ...
    private static final int FLING_MAX_VELOCITY=100;
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float x, float y) {
        if (e2.getX()-e1.getX()>FLING_MAX_VELOCITY && Math.abs(x)>FLING_MAX_VELOCITY){
            viewFlipper.setInAnimation(lInAnim);
            viewFlipper.setOutAnimation(rOutAnim);
            viewFlipper.showPrevious();
            setTitle("相片编号:"+viewFlipper.getDisplayedChild());
            return true;
        }else if(e1.getX()-e2.getX()>FLING_MAX_VELOCITY && Math.abs(x)>FLING_MAX_VELOCITY ){
